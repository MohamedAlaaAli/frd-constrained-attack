...
from frd.utils import frechet_distance

from frd.radiomics_utils import (
    convert_radiomic_dfs_to_vectors,
    compute_and_save_imagefolder_radiomics,
    compute_and_save_imagefolder_radiomics_parallel,
    interpret_radiomic_differences,
    compute_batch_radiomics,
)
...
def extract_features_from_batch(image_batch, parallelize=True, to_grayscale=True):
    ...
    if to_grayscale and batch_np.shape[1] == 3:
        batch_np = (
            0.299 * batch_np[:, 0] + 0.587 * batch_np[:, 1] + 0.114 * batch_np[:, 2]
        )
    ...
    features, _ = convert_radiomic_dfs_to_vectors(
        radiomics_df, radiomics_df, match_sample_count=True
    )
...
def get_distance_fn(image_batch, parallelize=True, to_grayscale=False):
    ...
    valid_feature_mask = ~np.isnan(batch_features).any(axis=0) & ~np.isinf(
        batch_features
    ).any(axis=0)
    ...
    def distance_fn(image):
        ...
        radiomics_df = radiomics_df[
            ["img_fname"]
            + [col for col in radiomics_df.columns if col in valid_feature_names]
        ]
        ...
        feature_indices = [
            radiomics_df.columns.get_loc(col) - 1 for col in valid_feature_names
        ]  # -1 for img_fname
...
def main(
    ...
    if batch1_np.shape[1] == 3:  # RGB
        ...
        batch1_np = (
            0.299 * batch1_np[:, 0] + 0.587 * batch1_np[:, 1] + 0.114 * batch1_np[:, 2]
        )
        batch1_np = batch1_np[:, np.newaxis, :, :]  # Add channel dimension back

    if batch2_np.shape[1] == 3:  # RGB
        batch2_np = (
            0.299 * batch2_np[:, 0] + 0.587 * batch2_np[:, 1] + 0.114 * batch2_np[:, 2]
        )
    ...
    if comp:
        ...
        if interpret:
            ...
            run_tsne = True
            interpret_radiomic_differences(
                radiomics_path1, radiomics_path2, run_tsne=run_tsne
            )
...
def main_folders(
    ...
    if force_compute_fresh or not os.path.exists(radiomics_path1):
        print(
            "No radiomics found computed for image folder 1 at {}, computing now.".format(
                radiomics_path1
            )
        )
        if parallelize:
            compute_and_save_imagefolder_radiomics_parallel(
                image_folder1, radiomics_fname=radiomics_fname
            )
        else:
            compute_and_save_imagefolder_radiomics(
                image_folder1, radiomics_fname=radiomics_fname
        ...
        print(
            "Radiomics already computed for image folder 1 at {}.".format(
                radiomics_path1
            )
        )
    ...
    if force_compute_fresh or not os.path.exists(radiomics_path2):
        print(
            "No radiomics found computed for image folder 2 at {}, computing now.".format(
                radiomics_path2
            )
        )
        if parallelize:
            compute_and_save_imagefolder_radiomics_parallel(
                image_folder2, radiomics_fname=radiomics_fname
            )
        else:
            compute_and_save_imagefolder_radiomics(
                image_folder2, radiomics_fname=radiomics_fname
        ...
        print(
            "Radiomics already computed for image folder 2 at {}.".format(
                radiomics_path2
            )
        )
    if comp:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force_compute_fresh",
        action="store_true",
        help="re-compute all radiomics fresh",
    )
    parser.add_argument(
        "--interpret",
        action="store_true",
        help="interpret the features underlying Fréchet Radiomics Distance",
    )
